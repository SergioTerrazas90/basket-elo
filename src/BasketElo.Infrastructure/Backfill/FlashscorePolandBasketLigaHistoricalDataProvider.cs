...
/// <summary>
/// Imports historical Polish top-flight results from Flashscore's paginated results feed.
/// The page contains the first results batch; the browser's "Show more matches" action
/// loads the remaining batches through the tr_* feed.
/// </summary>
public sealed class FlashscorePolandBasketLigaHistoricalDataProvider(HttpClient httpClient) : IBasketballDataProvider
{
    public const string Source = "flashscore-poland-plk";
    public const string ParserVersion = "flashscore-poland-plk-results-v1";
        ...
        CancellationToken cancellationToken)
    {
        if (!country.Equals("Poland", StringComparison.OrdinalIgnoreCase) ||
            (!leagueName.Equals("Tauron Basket Liga", StringComparison.OrdinalIgnoreCase) &&
             !leagueName.Equals("PLK", StringComparison.OrdinalIgnoreCase) &&
             !leagueName.Equals("Basket Liga", StringComparison.OrdinalIgnoreCase)))
        {
            return Task.FromResult<BasketballProviderLeague?>(null);
        }

        return Task.FromResult<BasketballProviderLeague?>(
            new BasketballProviderLeague(Source, "plk", "PLK", "PL", "literal"));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            warnings.Add($"Flashscore Polish PLK coverage is configured for 2001-2002 through 2007-2008; {season} is outside that range.");
            return ([], false, warnings);
        }

        if (!context.CanUseRequest())
        {
            return ([], true, ["The request budget was exhausted before loading the Flashscore Polish PLK page."]);
        }

        context.ConsumeRequest();
        var pagePath = $"/basketball/poland/basket-liga-{startYear}-{startYear + 1}/results/";
        ...
        if (initial is null)
        {
            warnings.Add($"Flashscore did not expose the paginated Polish PLK results feed for {season}.");
            return ([], false, warnings);
        }
        ...
        {
            warnings.Add($"Flashscore omitted one or more pagination identifiers for Polish PLK {season}.");
        ...
        while (games.Count < initial.Value.AllEventsCount && dataPart <= 32)
        {
            if (!context.CanUseRequest())
            {
                warnings.Add($"The request budget stopped Flashscore Polish PLK pagination at batch {dataPart} for {season}.");
        ...
        if (games.Count == 0)
        {
            warnings.Add($"Flashscore did not expose any complete finished Polish PLK games for {season}.");
        }
        else if (games.Count < initial.Value.AllEventsCount)
        {
            warnings.Add($"Flashscore exposed {games.Count} of {initial.Value.AllEventsCount} listed Polish PLK games for {season}.");
        }
    ...
    private static void AddParsedGames(string feed, string season, string pageUrl, DateTime fetchedAtUtc, string revision, IDictionary<string, BasketballProviderGame> games)
    {
    ...
    private static BasketballProviderGame CreateGame(string eventId, string season, string pageUrl, DateTime fetchedAtUtc, string revision, long timestamp, string homeName, string awayName, short homeScore, short awayScore, IReadOnlyDictionary<string, string> fields)
    {
        ...
        return new BasketballProviderGame(Source, eventId, DateTime.SpecifyKind(date.AddHours(12), DateTimeKind.Utc), "finished", $"flashscore-team:{homeId}", homeName, $"flashscore-team:{awayId}", awayName, homeScore, awayScore, new BasketballProviderGameProvenance(pageUrl, season, fetchedAtUtc, ParserVersion, revision), CompetitionPhase: "PLK", CompetitionRound: round);
    }
    ...
    private static (string Data, int AllEventsCount, int SeasonId)? ParseInitialResults(string html)
    {
        ...
        if (!match.Success || !int.TryParse(match.Groups["count"].Value, NumberStyles.Integer, CultureInfo.InvariantCulture, out var count) || !int.TryParse(match.Groups["season"].Value, NumberStyles.Integer, CultureInfo.InvariantCulture, out var seasonId))
        {
            return null;
        }
    ...
    private static bool TryParseSeason(string season, out int startYear)
    {
        ...
        return parts.Length == 2 && int.TryParse(parts[0], NumberStyles.Integer, CultureInfo.InvariantCulture, out startYear) && int.TryParse(parts[1], NumberStyles.Integer, CultureInfo.InvariantCulture, out var endYear) && endYear == startYear + 1;
    }
}
