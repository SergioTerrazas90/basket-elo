...
[Migration("20260808094500_ReconcileFibaAsiaCupGames")]
public partial class ReconcileFibaAsiaCupGames : Migration
{
    protected override void Up(MigrationBuilder migrationBuilder)
    {
        migrationBuilder.Sql("""
            ...
            BEGIN
                CREATE TEMP TABLE fiba_asia_reconciliation_candidates ON COMMIT DROP AS
                ...
                WHERE (
                    fiba_competition."Name" = 'FIBA Asia Cup'
                    AND gsa_competition."Name" = 'FIBA Asia Cup'
                )
                OR (
                    fiba_competition."Name" IN ('FIBA Asia Cup Qualifiers', 'FIBA Asia Cup Pre-Qualifiers')
                    AND gsa_competition."Name" = 'FIBA Asia Cup Qualification'
                );

                IF EXISTS (
                    SELECT 1
                    FROM fiba_asia_reconciliation_candidates
                    GROUP BY fiba_id
                    HAVING count(*) > 1
                ) OR EXISTS (
                    SELECT 1
                    ...
                    RAISE EXCEPTION 'FIBA Asia reconciliation is not one-to-one; no rows were removed';
                END IF;

                DELETE FROM games gsa_game
                USING fiba_asia_reconciliation_candidates candidate
                WHERE gsa_game."Id" = candidate.gsa_id
                  AND gsa_game."Source" = 'global-sports-archive'
                  AND NOT gsa_game."HasManualResultOverride";

                GET DIAGNOSTICS reconciled_count = ROW_COUNT;
                RAISE NOTICE 'Removed % exact GSA duplicates; FIBA Asia rows remain canonical', reconciled_count;
            END $$;
            """);
    }
